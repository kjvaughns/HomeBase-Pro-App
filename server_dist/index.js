...
__export(schema_exports, {
  ...
  reviewReportsRelations: () => reviewReportsRelations,
  reviews: () => reviews,
  reviewsRelations: () => reviewsRelations,
  savedProviders: () => savedProviders,
  savedProvidersRelations: () => savedProvidersRelations,
...
var init_schema = __esm({
  "shared/schema.ts"() {
    ...
    jobStatusEnum = pgEnum("job_status", [
      "scheduled",
      "confirmed",
      "on_my_way",
      "arrived",
    ...
    payoutStatusEnum = pgEnum("payout_status", [
      "pending",
      "in_transit",
      "paid",
      "failed"
    ]);
    connectOnboardingStatusEnum = pgEnum("connect_onboarding_status", [
      "not_started",
      "pending",
      "complete"
    ]);
    providerPlanTierEnum = pgEnum("provider_plan_tier", [
      "free",
      "starter",
      "professional",
      "enterprise"
    ...
    serviceCategoriesRelations = relations(
      serviceCategories,
      ({ many }) => ({
    ...
    providerServicesRelations = relations(
      providerServices,
      ({ one }) => ({
        provider: one(providers, {
    ...
    pricingTypeEnum = pgEnum("pricing_type", [
      "fixed",
      "variable",
      "service_call",
      "quote"
    ...
    providerCustomServicesRelations = relations(
    ...
    insertProviderCustomServiceSchema = createInsertSchema(
      providerCustomServices
    ).omit({
      id: true,
      createdAt: true,
    ...
    appointments = pgTable(
      ...
      (table) => ({
        ...
        userProviderSlotUnique: uniqueIndex("appointments_user_provider_slot_unique").on(table.userId, table.providerId, table.scheduledDate).where(sql`user_id IS NOT NULL AND scheduled_date IS NOT NULL`)
      })
    );
    ...
    savedProvidersRelations = relations(savedProviders, ({ one }) => ({
    ...
    reviewReportsRelations = relations(reviewReports, ({ one }) => ({
    ...
    maintenanceReminderFrequencyEnum = pgEnum(
    ...
    maintenanceRemindersRelations = relations(
    ...
    stripeConnectAccountsRelations = relations(
    ...
    stripeWebhookEvents = pgTable("stripe_webhook_events", {
      ...
      // 'platform' | 'connect' (nullable for legacy rows)
      stripeAccountId: text("stripe_account_id"),
      // event.account, null on platform events
    ...
    clients = pgTable("clients", {
      ...
      homeData: text("home_data"),
      // JSON: legacy HouseFax cache (deprecated in favor of homeId)
      homeId: varchar("home_id").references(() => homes.id, {
        onDelete: "set null"
      }),
    ...
    payments = pgTable("payments", {
      ...
      paymentIntentUnique: uniqueIndex("payments_stripe_payment_intent_id_unique").on(table.stripePaymentIntentId)
    }));
    ...
    bookingLinkStatusEnum = pgEnum("booking_link_status", [
      "active",
      "paused",
      "disabled"
    ]);
    quoteModeEnum = pgEnum("quote_mode", [
      "range",
      "fixed",
      "estimate_after_review"
    ]);
    intakeStatusEnum = pgEnum("intake_status", [
      "submitted",
      "confirmed",
      "reviewed",
      "converted",
    ...
    bookingLinksRelations = relations(
      bookingLinks,
      ({ one, many }) => ({
        provider: one(providers, {
    ...
    intakeSubmissionsRelations = relations(
      intakeSubmissions,
      ({ one }) => ({
    ...
    homeProfileUpdateSchema = createInsertSchema(homes).pick({
      propertyType: true,
      bedrooms: true,
      bathrooms: true,
      squareFeet: true,
    ...
    homeFieldChanges = pgTable("home_field_changes", {
      id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
      homeId: varchar("home_id").notNull().references(() => homes.id, { onDelete: "cascade" }),
      ...
      // "zillow_import" | "homeowner" | "health_score" | "survival_kit" | "provider"
      changedByUserId: varchar("changed_by_user_id").references(() => users.id, {
        onDelete: "set null"
      }),
      changedAt: timestamp("changed_at").defaultNow().notNull()
    });
    homeFieldChangesRelations = relations(
      homeFieldChanges,
      ({ one }) => ({
        home: one(homes, {
    ...
    insertStripeConnectAccountSchema = createInsertSchema(
      stripeConnectAccounts
    ).omit({
      id: true,
      createdAt: true,
    ...
    insertInvoiceLineItemSchema = createInsertSchema(
      invoiceLineItems
    ).omit({
      id: true,
      createdAt: true
    ...
    insertIntakeSubmissionSchema = createInsertSchema(
      intakeSubmissions
    ).omit({
      id: true,
      createdAt: true,
    ...
    notificationDeliveryStatusEnum = pgEnum(
    ...
    providerMessagesRelations = relations(
      providerMessages,
      ({ one }) => ({
        provider: one(providers, {
    ...
    insertProviderMessageSchema = createInsertSchema(
      providerMessages
    ).omit({
      id: true,
      createdAt: true
    ...
    messageTemplatesRelations = relations(
      messageTemplates,
      ({ one }) => ({
        provider: one(providers, {
    ...
    insertMessageTemplateSchema = createInsertSchema(
      messageTemplates
    ).omit({
      id: true,
      createdAt: true,
    ...
    insertNotificationPreferenceSchema = createInsertSchema(
    ...
    housefaxEntriesRelations = relations(
      housefaxEntries,
      ({ one }) => ({
        home: one(homes, {
    ...
    insertHousefaxEntrySchema = createInsertSchema(
      housefaxEntries
    ).omit({
      id: true,
      createdAt: true
...
__export(emailService_exports, {
  ...
  sendProviderScheduledJobEmail: () => sendProviderScheduledJobEmail,
  sendRebookingNudgeEmail: () => sendRebookingNudgeEmail,
  sendReviewReplyEmail: () => sendReviewReplyEmail,
  ...
  sendSubscriptionExpiredEmail: () => sendSubscriptionExpiredEmail,
  sendSubscriptionGraceReminderEmail: () => sendSubscriptionGraceReminderEmail,
  sendSubscriptionGraceStartEmail: () => sendSubscriptionGraceStartEmail,
...
async function sendWelcomeEmail(to, name) {
  const body = greeting(name) + paragraph(
  ...
  ) + `<h2 style="color:#111827;font-size:18px;margin:0 0 12px;">What you can do</h2><ul style="color:#6b7280;font-size:14px;line-height:1.8;padding-left:20px;margin:0 0 20px;">
...
async function sendProviderOnboardingCompleteEmail(to, providerName) {
  const body = greeting(providerName) + paragraph(
...
async function sendBookingConfirmationEmail(data) {
  ...
  const issueSection = data.description ? `<div style="background:#f0fdf4;border-radius:8px;padding:16px;margin-bottom:20px;border-left:4px solid #38AE5F;">
  ...
  const intakeSection = data.intakeAnswers ? `<div style="background:#f0fdf4;border-radius:8px;padding:16px;margin-bottom:20px;border-left:4px solid #38AE5F;">
  ...
  const addOnsSection = data.addOns && data.addOns.length > 0 ? `<div style="background:#f9fafb;border-radius:8px;padding:14px 16px;margin-bottom:20px;">
  ...
  const serviceDescSection = data.serviceDescription ? `<div style="background:#f9fafb;border-radius:8px;padding:14px 16px;margin-bottom:20px;">
  ...
  ) + issueSection + intakeSection + addOnsSection + serviceDescSection + nextStepsSection + `<div style="background:#fffbeb;border-radius:8px;padding:14px 16px;margin-bottom:20px;border-left:4px solid #f59e0b;">
...
async function sendBookingRescheduledEmail(data, oldDate, oldTime) {
  ...
  ) + `<p style="color:#6b7280;font-size:13px;text-decoration:line-through;margin:0 0 4px;">Previously: ${escapeHtml(oldDate)} at ${escapeHtml(oldTime)}</p>` + infoBox(
...
async function sendBookingRequestReceivedEmail(data) {
  const issueSection = data.description ? `<div style="background:#f0fdf4;border-radius:8px;padding:16px;margin-bottom:20px;border-left:4px solid #38AE5F;">
  ...
  const body = greeting(data.clientName) + paragraph(
    ...
    (data.confirmationNumber ? infoRow("Reference #", data.confirmationNumber) : "") + (data.serviceName ? infoRow("Service", data.serviceName) : "") + infoRow("Provider", data.providerName) + (data.preferredDate ? infoRow("Preferred Date", data.preferredDate) : "") + (data.preferredTime ? infoRow("Preferred Time", data.preferredTime) : "") + (data.address ? infoRow("Location", data.address) : "")
  ) + issueSection + `<div style="background:#fffbeb;border-radius:8px;padding:14px 16px;margin-bottom:20px;border-left:4px solid #f59e0b;">
...
async function sendReviewReplyEmail(data) {
  ...
  return sendEmail(
    ...
    buildEmailBase(
      "Review Reply",
      body,
      data.reviewUrl ? "Open in HomeBase" : void 0,
      data.reviewUrl
    )
  );
}
...
async function sendPaymentFailedEmail(data) {
  ...
  ) + (data.paymentLink ? `<a href="${data.paymentLink}" style="display:block;background:#38AE5F;color:#fff;text-align:center;padding:16px 32px;border-radius:8px;text-decoration:none;font-weight:600;font-size:16px;margin-bottom:24px;">Retry Payment</a>` : "") + appDownloadSection();
  return sendEmail(
    data.clientEmail,
...
async function sendReviewRequestEmail(data) {
  ...
  return sendEmail(
    ...
    buildEmailBase(
      "Leave a Review",
      body,
      data.reviewUrl ? "Leave a Review" : void 0,
      data.reviewUrl
    )
  );
}
...
async function sendSubscriptionGraceStartEmail(data) {
  const body = greeting(data.providerName) + paragraph(
...
async function sendSubscriptionGraceReminderEmail(data) {
...
async function sendSubscriptionExpiredEmail(data) {
  const body = greeting(data.providerName) + paragraph(
...
async function sendIntakeSubmissionNotification(data) {
  const body = greeting(data.providerName) + paragraph(
    ...
    `<div style="margin-bottom:14px;">
      ...
      </div>` + (data.clientEmail ? `<div style="margin-bottom:14px;"><span style="color:#6b7280;font-size:11px;text-transform:uppercase;font-weight:500;">Email</span><p style="color:#111827;margin:3px 0 0;"><a href="mailto:${escapeHtml(data.clientEmail)}" style="color:#38AE5F;">${escapeHtml(data.clientEmail)}</a></p></div>` : "") + (data.clientPhone ? `<div style="margin-bottom:14px;"><span style="color:#6b7280;font-size:11px;text-transform:uppercase;font-weight:500;">Phone</span><p style="color:#111827;margin:3px 0 0;"><a href="tel:${escapeHtml(data.clientPhone)}" style="color:#38AE5F;">${escapeHtml(data.clientPhone)}</a></p></div>` : "") + (data.address ? `<div style="margin-bottom:14px;"><span style="color:#6b7280;font-size:11px;text-transform:uppercase;font-weight:500;">Address</span><p style="color:#111827;margin:3px 0 0;">${escapeHtml(data.address)}</p></div>` : "") + `<div><span style="color:#6b7280;font-size:11px;text-transform:uppercase;font-weight:500;">Problem Description</span><p style="color:#111827;margin:3px 0 0;line-height:1.5;">${escapeHtml(data.problemDescription)}</p></div>`
...
async function sendProviderScheduledJobEmail(data) {
  ...
  const priceRow = data.estimatedPrice ? infoRow(
    priceLabel,
    fmtUsd(
      typeof data.estimatedPrice === "string" ? parseFloat(data.estimatedPrice) : data.estimatedPrice
    )
  ) : isQuoteType ? infoRow("Pricing", "Quote provided after review") : "";
  const issueSection = data.description ? `<div style="background:#f0fdf4;border-radius:8px;padding:16px;margin-bottom:20px;border-left:4px solid #38AE5F;">
  ...
  const contactRows = [
    data.providerPhone ? `<span style="color:#6b7280;font-size:13px;">${escapeHtml(data.providerPhone)}</span>` : "",
    data.providerEmail ? `<span style="color:#6b7280;font-size:13px;">${escapeHtml(data.providerEmail)}</span>` : ""
  ].filter(Boolean).join(" &bull; ");
  const contactSection = contactRows ? `<div style="background:#fffbeb;border-radius:8px;padding:14px 16px;margin-bottom:20px;border-left:4px solid #f59e0b;">
      ...
      </div>` : `<div style="background:#fffbeb;border-radius:8px;padding:14px 16px;margin-bottom:20px;border-left:4px solid #f59e0b;">
        <p style="color:#92400e;font-size:13px;margin:0;"><strong>Need to reschedule?</strong> Contact ${escapeHtml(data.providerName)} at least 24 hours before your appointment.</p>
      </div>`;
  const body = greeting(data.clientName) + paragraph(
    `Your appointment has been confirmed by <strong>${escapeHtml(data.providerName)}</strong>. Here are the details:`
  ) + infoBox(
    infoRow("Service", data.serviceName) + infoRow("Provider", data.providerName) + infoRow("Date", data.scheduledDate) + (data.scheduledTime ? infoRow("Time", data.scheduledTime) : "") + (data.address ? infoRow("Location", data.address) : "") + priceRow
  ) + issueSection + addOnsSection + serviceDescSection + nextSteps + contactSection + appDownloadSection();
  return sendEmail(
    data.clientEmail,
...
async function sendSupportTicketEmail(data) {
  try {
    const { client, fromEmail } = await getResendClient();
    const body = greeting(data.name) + paragraph(
...
var init_emailService = __esm({
  "server/emailService.ts"() {
    "use strict";
    SUBSCRIBE_DEEP_LINK = "homebase://Subscription";
  }
});
...
__export(notificationService_exports, {
  claimNotificationDelivery: () => claimNotificationDelivery,
  dispatch: () => dispatch,
  dispatchNotification: () => dispatchNotification,
  dispatchWithResult: () => dispatchWithResult,
  hasDeliveryForRecord: () => hasDeliveryForRecord,
...
async function claimNotificationDelivery(eventType, dedupKey, channel) {
  try {
      ...
      if (clientEmail && clientName) {
        ...
        if (emailOptedOut) {
          ...
          try {
            ...
            await updateDelivery(deliveryId, "failed", void 0, err?.message);
...
var EVENT_PREF_FIELD, REVIEW_NUDGE_EVENT, REVIEW_NUDGE_REVIEWABLE_STATUSES;
var init_notificationService = __esm({
  "server/notificationService.ts"() {
    ...
    REVIEW_NUDGE_EVENT = "review.nudge";
    REVIEW_NUDGE_REVIEWABLE_STATUSES = /* @__PURE__ */ new Set([
      "completed",
      "paid",
      "closed",
      "awaiting_payment"
...
__export(subscriptionService_exports, {
  ...
  checkSubscriptionGate: () => checkSubscriptionGate,
  computeSubscriptionStatus: () => computeSubscriptionStatus,
  getProviderSubscriptionStatus: () => getProviderSubscriptionStatus,
  maybeStartGracePeriod: () => maybeStartGracePeriod,
  sendGraceExpiredNotification: () => sendGraceExpiredNotification,
...
function computeSubscriptionStatus(plan, now = /* @__PURE__ */ new Date()) {
  ...
  const graceEndsAt = plan?.gracePeriodEndsAt ? new Date(plan.gracePeriodEndsAt) : null;
  const subscriptionSource = plan?.subscriptionSource ?? null;
  const currentPeriodEnd = plan?.currentPeriodEnd ? new Date(plan.currentPeriodEnd).toISOString() : null;
  if (plan?.isPartner) {
    return {
      status: "subscribed",
  ...
  const daysRemaining = Math.max(
    0,
    Math.ceil(msRemaining / (1e3 * 60 * 60 * 24))
  );
  return {
    status: "grace_period",
...
async function checkSubscriptionGate(providerId, res) {
  ...
  if (info.status === "expired") {
    res.status(403).json({
      error: "Your HomeBase subscription is required to continue. Open the Subscription screen to subscribe and unlock job and invoice creation.",
...
async function sendGraceStartNotification(providerId) {
...
async function sendGraceReminderNotification(providerId, daysRemaining) {
  const ctx = await getProviderUser(providerId);
  if (!ctx) return;
  const { provider, user } = ctx;
...
async function sendGraceExpiredNotification(providerId) {
  const ctx = await getProviderUser(providerId);
  if (!ctx) return;
  const { provider, user } = ctx;
...
// server/stripeWebhookRouter.ts
var stripeWebhookRouter_exports = {};
__export(stripeWebhookRouter_exports, {
  expectedEndpointFor: () => expectedEndpointFor,
...
async function getHandlers() {
  ...
  const subSvc = await Promise.resolve().then(() => (init_subscriptionService(), subscriptionService_exports)).catch(() => null);
  handlersCache = {
    "account.updated": (e) => svc.handleAccountUpdated(e.data.object),
    "payment_intent.succeeded": (e) => svc.handlePaymentIntentSucceeded(e.data.object),
    "payment_intent.payment_failed": (e) => svc.handlePaymentIntentFailed(e.data.object),
    "charge.refunded": (e) => svc.handleChargeRefunded(e.data.object),
    "payout.created": (e) => svc.handlePayoutCreated(
      e.data.object,
      e.account ?? null
    ),
    "payout.paid": (e) => svc.handlePayoutPaid(
      e.data.object,
      e.account ?? null
    ),
    "payout.failed": (e) => svc.handlePayoutFailed(
      e.data.object,
      e.account ?? null
    ),
    "checkout.session.completed": async (e) => {
      const session = e.data.object;
      if (session.mode === "subscription" || session.metadata?.subscriptionType === "homebase_pro") {
        await svc.handleSubscriptionCheckoutCompleted(session);
      } else {
        await svc.handleCheckoutSessionCompleted(session);
      }
    },
    "customer.subscription.updated": (e) => svc.handleSubscriptionUpdated(e.data.object),
    "customer.subscription.deleted": (e) => svc.handleSubscriptionDeleted(e.data.object),
    ...
    "invoice.payment_failed": async (e) => {
      const inv = e.data.object;
      if (inv.subscription || inv.billing_reason === "subscription_cycle" || inv.billing_reason === "subscription_create") {
...
async function processStripeEvent(event, endpoint) {
  ...
  const expectedEndpoint = account ? "connect" : "platform";
...
function expectedEndpointFor(event) {
  return event.account ? "connect" : "platform";
}
async function reserveEvent(event, endpoint) {
  try {
...
var handlersCache;
var init_stripeWebhookRouter = __esm({
  "server/stripeWebhookRouter.ts"() {
    "use strict";
    init_db();
    init_schema();
...
var stripeConnectService_exports = {};
__export(stripeConnectService_exports, {
  __setStripeForTesting: () => __setStripeForTesting,
  ...
  createDirectCheckoutSession: () => createDirectCheckoutSession,
  ...
  createSubscriptionPortalSession: () => createSubscriptionPortalSession,
  getConnectAccount: () => getConnectAccount,
  getConnectStatus: () => getConnectStatus,
  getProviderPlan: () => getProviderPlan,
  getProviderReadinessSet: () => getProviderReadinessSet,
  getStripe: () => getStripe,
  handleAccountUpdated: () => handleAccountUpdated,
  handleChargeRefunded: () => handleChargeRefunded,
  handleCheckoutSessionCompleted: () => handleCheckoutSessionCompleted,
  handlePaymentIntentFailed: () => handlePaymentIntentFailed,
  handlePaymentIntentSucceeded: () => handlePaymentIntentSucceeded,
  handlePayoutCreated: () => handlePayoutCreated,
  handlePayoutFailed: () => handlePayoutFailed,
  handlePayoutPaid: () => handlePayoutPaid,
  handleStripeInvoicePaid: () => handleStripeInvoicePaid,
  handleStripeInvoicePaymentFailed: () => handleStripeInvoicePaymentFailed,
  handleStripeWebhook: () => handleStripeWebhook,
  handleSubscriptionCheckoutCompleted: () => handleSubscriptionCheckoutCompleted,
  handleSubscriptionDeleted: () => handleSubscriptionDeleted,
  handleSubscriptionInvoicePaymentFailed: () => handleSubscriptionInvoicePaymentFailed,
  handleSubscriptionUpdated: () => handleSubscriptionUpdated,
  isProviderReadyForCharges: () => isProviderReadyForCharges,
  isStripeLiveMode: () => isStripeLiveMode,
  refreshConnectAccountLink: () => refreshConnectAccountLink,
  reonboardConnectAccount: () => reonboardConnectAccount,
...
function getStripe() {
  if (!stripe) {
    ...
    const apiKey = isProd ? process.env.STRIPE_SECRET_KEY : process.env.STRIPE_TEST_SECRET_KEY || process.env.STRIPE_SECRET_KEY;
    if (!apiKey) {
      throw new Error(
        isProd ? "STRIPE_SECRET_KEY is required in production. Set the live secret key in the deployment env." : "STRIPE_TEST_SECRET_KEY or STRIPE_SECRET_KEY is required. Please add it to your environment variables."
      );
    }
...
function __setStripeForTesting(client) {
  stripe = client;
}
...
async function getConnectStatus(providerId) {
  ...
  try {
    ...
    if (code === "account_invalid" || code === "resource_missing" || err?.statusCode === 404) {
      console.warn(
        `[stripe-connect] accounts.retrieve(${connectAccount.stripeAccountId}) failed \u2014 likely a ${isStripeLiveMode() ? "test-mode" : "live-mode"} account under ${isStripeLiveMode() ? "live" : "test"} API key. Provider must re-onboard.`
      );
      return {
        exists: true,
        accountId: connectAccount.stripeAccountId,
...
async function createStripeInvoice(invoiceId) {
  ...
  const daysUntilDue = invoice.dueDate ? Math.max(
    1,
    Math.ceil(
      (new Date(invoice.dueDate).getTime() - Date.now()) / 864e5
    )
  ) : 30;
  if (typeof connectId !== "string" || !connectId.startsWith("acct_") || connectId.length < 10) {
    throw new Error(
...
async function createDirectCheckoutSession(_invoiceId, _reqHost) {
  throw new Error(
...
async function handleStripeWebhook(event) {
  const { processStripeEvent: processStripeEvent2 } = await Promise.resolve().then(() => (init_stripeWebhookRouter(), stripeWebhookRouter_exports));
...
async function handlePaymentIntentSucceeded(paymentIntent) {
  ...
  const piInvoice = resolveExpandableId(
  ...
  if (updatedInvoice) {
    try {
      ...
      await maybeStartGracePeriod2(updatedInvoice.providerId);
    } catch (e) {
      ...
      if (clientEmail && provider) {
        ...
        if (emailClaimed) {
          dispatch("invoice.paid", {
            ...
            (e) => console.error("invoice.paid dispatch error (webhook):", e)
          );
        }
      }
      if (provider?.userId) {
        ...
        if (providerPushClaimed) {
          ...
          dispatchNotification(
            ...
            (e) => console.error("provider invoice.paid push error (webhook):", e)
          );
        }
      }
      if (updatedInvoice.homeownerUserId) {
        ...
        if (homeownerPushClaimed) {
          ...
          dispatchNotification(
            ...
            () => logDelivery({
              ...
              recipientUserId: updatedInvoice.homeownerUserId ?? void 0,
              relatedRecordType: "invoice",
              relatedRecordId: paymentIntent.id
            })
          ).catch(
            (e) => console.error("homeowner invoice.paid push error (webhook):", e)
          );
        }
      }
      (async () => {
        try {
          ...
          if (!jobId) return;
          const costCents = updatedInvoice.total ? Math.round(
            (typeof updatedInvoice.total === "string" ? parseFloat(updatedInvoice.total) : updatedInvoice.total) * 100
          ) : 0;
      ...
      if (failedInvoice) {
        ...
        if (clientEmail && provider) {
          dispatch("invoice.payment_failed", {
            ...
            (e) => console.error("invoice.payment_failed dispatch error:", e)
          );
        }
      }
    } catch (err) {
...
async function handleStripeInvoicePaid(stripeInvoice) {
  ...
  const invoiceWithLinks = stripeInvoice;
  const stripePaymentIntentId = resolveExpandableId(
    invoiceWithLinks.payment_intent
  );
  const stripeChargeId = resolveExpandableId(
...
async function handleCheckoutSessionCompleted(session) {
  ...
  const cancellationFeeForAppointment = session.metadata?.cancellationFeeForAppointment;
  ...
  try {
    ...
    if (bookedUser && bookedProvider) {
      await dispatch("booking.created", {
        ...
        estimatedPrice: appt.estimatedPrice ? parseFloat(String(appt.estimatedPrice)) || void 0 : void 0,
...
async function getOrCreateUserStripeCustomer(userId) {
  ...
  if (user.stripeCustomerId) {
    try {
      const existing = await stripe2.customers.retrieve(user.stripeCustomerId);
      if (!existing.deleted) return user.stripeCustomerId;
    } catch {
    }
  }
  const fullName = [user.firstName, user.lastName].filter(Boolean).join(" ") || void 0;
...
function planTierForStatus(status) {
...
async function handleSubscriptionUpdated(subscription) {
  ...
  const currentPeriodEnd = typeof periodEndUnix === "number" ? new Date(periodEndUnix * 1e3) : null;
  await upsertProviderPlanSubscription(providerId, {
    planTier: planTierForStatus(status),
    isSubscribed: isActive,
    stripeSubscriptionId: subscription.id,
    subscriptionStatus: status,
    subscriptionEndedAt: isActive ? null : /* @__PURE__ */ new Date(),
    subscriptionSource: "stripe_web",
    currentPeriodEnd
  });
}
async function handleSubscriptionDeleted(subscription) {
  ...
  await upsertProviderPlanSubscription(providerId, {
    ...
    subscriptionEndedAt: /* @__PURE__ */ new Date(),
    subscriptionSource: "stripe_web"
  });
  await notifyProviderUser(
    providerId,
...
async function handleSubscriptionInvoicePaymentFailed(stripeInvoice) {
...
var stripe, APP_URL, PUBLIC_REDIRECT_BASE, SUBSCRIPTION_RETURN_BASE;
var init_stripeConnectService = __esm({
  "server/stripeConnectService.ts"() {
    "use strict";
    init_db();
    init_schema();
...
var init_auth = __esm({
  "server/auth.ts"() {
    ...
    if (!process.env.JWT_SECRET) {
      if (IS_PROD) {
        console.error(
          "[auth] CRITICAL: JWT_SECRET is not set in the production environment. Refusing to start. Set JWT_SECRET in Replit Secrets and redeploy."
        );
        process.exit(1);
      } else {
...
// server/providerResources.ts
var providerResources_exports = {};
__export(providerResources_exports, {
...
function sanitizeResource(raw) {
  ...
  const r = raw;
  if (!isString(r.id) || !isString(r.icon) || !isString(r.title) || !isString(r.description) || !isString(r.type) || !isString(r.url)) {
    return null;
  }
  if (!ALLOWED_TYPES.has(r.type)) return null;
...
function sanitizeFeed(payload) {
  ...
  if (Array.isArray(payload)) {
    arr = payload;
  } else if (payload && typeof payload === "object" && Array.isArray(payload.resources)) {
    arr = payload.resources;
  } else {
    return null;
  }
  const cleaned = [];
...
async function getProviderResources() {
  const now = Date.now();
  if (cache && now - cache.fetchedAt < FRESH_TTL_MS) {
    return { ...cache };
  }
  const recentlyAttempted = now - lastUpstreamAttemptAt;
  const onBundledBackoff = cache?.source === "bundled" && recentlyAttempted < BUNDLED_BACKOFF_MS;
  const onStaleUpstreamBackoff = cache?.source === "upstream" && cache.fetchedAt < lastUpstreamAttemptAt && recentlyAttempted < STALE_UPSTREAM_BACKOFF_MS;
...
var ALLOWED_TYPES, BUNDLED_FALLBACK, FEED_URL, FRESH_TTL_MS, BUNDLED_BACKOFF_MS, STALE_UPSTREAM_BACKOFF_MS, FETCH_TIMEOUT_MS, cache, lastUpstreamAttemptAt, inFlight;
var init_providerResources = __esm({
  "server/providerResources.ts"() {
    "use strict";
    ...
    BUNDLED_FALLBACK = [
      {
        ...
        description: "Complete your Business Hub profile, add services with clear pricing, create a public booking link, and connect Stripe to accept payments. A complete profile receives 3x more client inquiries.",
        type: "guide",
      ...
      },
      {
        ...
        description: "Research local market rates and factor in labor, materials, overhead, and a 15-20% profit margin. Update your pricing quarterly as material costs change to stay competitive and profitable.",
        type: "article",
      ...
      },
      {
        ...
        description: "Send a reminder message 24 hours before arrival, arrive on time, take before-and-after photos of your work, and follow up the next day. Satisfied clients become long-term repeat clients.",
        type: "article",
      ...
      },
      {
        ...
        description: "Use natural lighting and capture wide shots plus close-up detail of finished work. Before-and-after photos can increase booking rates by up to 40%. Keep your phone steady \u2014 a small tripod helps.",
        type: "guide",
      ...
      },
      {
        ...
        description: "Maintain at least $1M general liability coverage. Add workers' comp if you have employees, and a tools-and-equipment policy for high-value gear. Upload proof of insurance in your Business Hub to build trust.",
        type: "guide",
      ...
      },
      {
        ...
        description: "Your HomeBase dashboard tracks conversion rate, average job value, repeat-client rate, and total revenue. Review your stats weekly to spot trends and find opportunities to grow your business.",
        type: "tool",
      ...
      },
      {
        ...
        description: "Set your business hours to control when clients can book. Use minimum advance booking windows to avoid last-minute rushes, and block personal time to maintain work-life balance.",
        type: "article",
      ...
      },
      {
        ...
        description: "Connect Stripe in your Business Hub to enable online payments. Require a deposit on booking links for larger jobs. Send invoices within 1 hour of job completion \u2014 faster invoicing means faster payment.",
        type: "guide",
        url: "https://homebaseproapp.com/blog/getting-paid-faster"
      }
    ];
    FEED_URL = process.env.PROVIDER_RESOURCES_FEED_URL ?? "https://homebasepro-app.lovable.app/provider-resources.json";
    FRESH_TTL_MS = 5 * 60 * 1e3;
    BUNDLED_BACKOFF_MS = 30 * 1e3;
    STALE_UPSTREAM_BACKOFF_MS = 30 * 1e3;
    FETCH_TIMEOUT_MS = 5e3;
    cache = null;
    lastUpstreamAttemptAt = 0;
    inFlight = null;
  }
});

// server/revenuecatService.ts
var revenuecatService_exports = {};
...
function eventEntitlements(event) {
...
async function handleRevenueCatWebhook(req, res) {
  ...
  try {
    if (ACTIVATING_EVENTS.has(eventType)) {
      ...
      const stillEntitled = eventType === "CANCELLATION" && expiresAt !== null && expiresAt.getTime() > Date.now();
      const { wasSubscribed, isSubscribed } = await upsertPlan(providerId, {
        isSubscribed: stillEntitled ? true : false,
        subscriptionStatus: stillEntitled ? "cancel_at_period_end" : eventType.toLowerCase(),
        subscriptionEndedAt: stillEntitled ? null : /* @__PURE__ */ new Date(),
        currentPeriodEnd: expiresAt
      });
...
var ENTITLEMENT_ID, ACTIVATING_EVENTS, DEACTIVATING_EVENTS;
var init_revenuecatService = __esm({
  "server/revenuecatService.ts"() {
    "use strict";
    init_db();
    init_schema();
    init_notificationService();
    ENTITLEMENT_ID = "pro";
    ACTIVATING_EVENTS = /* @__PURE__ */ new Set([
      "INITIAL_PURCHASE",
      "RENEWAL",
      "UNCANCELLATION",
      "PRODUCT_CHANGE",
    ...
    DEACTIVATING_EVENTS = /* @__PURE__ */ new Set([
      "CANCELLATION",
      "EXPIRATION",
      "BILLING_ISSUE",
      "SUBSCRIPTION_PAUSED"
...
// shared/jobSummary.ts
function formatIntakeAnswers(answers, questions) {
...
function formatAddOnLine(addOn) {
  const name = (addOn.name ?? "").trim();
  if (!name) return "";
  const priceNum = addOn.price == null ? NaN : typeof addOn.price === "number" ? addOn.price : parseFloat(String(addOn.price));
  if (!isNaN(priceNum) && priceNum > 0) {
    return `${name} (+$${priceNum.toFixed(0)})`;
  }
  return name;
}
function formatJobSummary(input) {
  const sections = [];
...
${answerLines.join("\n")}`);
  ...
  const addOnLines = (input.addOns ?? []).map(formatAddOnLine).filter((s) => s.length > 0);
...
function parseIntakeAnswers(raw) {
...
function parseIntakeQuestions(raw) {
  if (raw == null) return [];
  let value = raw;
  if (typeof value === "string") {
    try {
  ...
  return value.filter(
    (q) => !!q && typeof q === "object" && typeof q.id === "string" && typeof q.question === "string"
  );
}
...
async function updateHomeWithChangeLog(params) {
  ...
  for (const [field, newValue] of Object.entries(updates)) {
    ...
    if (onlyIfEmpty && oldValue !== null && oldValue !== void 0) continue;
...
function policyHasAnyRule(policy) {
  return policy.requireDeposit && policy.depositPercent > 0 || policy.cancellationHours > 0 || policy.cancellationFeePercent > 0 || policy.rescheduleHours > 0 || policy.maxReschedules > 0;
}
...
function computeDepositCents(policy, estimatedTotalCents) {
...
function computeCancellationFee(policy, estimatedTotalCents, scheduledAt, now = /* @__PURE__ */ new Date()) {
  ...
  const hoursBefore = msUntil / (1e3 * 60 * 60);
  const inside = policy.cancellationHours > 0 && hoursBefore < policy.cancellationHours;
  if (!inside) {
...
function checkRescheduleAllowed(policy, scheduledAt, currentRescheduleCount, now = /* @__PURE__ */ new Date()) {
  if (policy.maxReschedules > 0 && currentRescheduleCount >= policy.maxReschedules) {
    return {
      allowed: false,
  ...
  if (policy.rescheduleHours > 0) {
    const scheduled = scheduledAt instanceof Date ? scheduledAt : scheduledAt ? new Date(scheduledAt) : null;
    if (scheduled && !isNaN(scheduled.getTime())) {
      const hoursBefore = (scheduled.getTime() - now.getTime()) / (1e3 * 60 * 60);
...
function createIpRateLimit(opts) {
  return (req, res, next) => {
    ...
    if (entry.count >= opts.limit) {
      res.status(429).json({
        error: opts.message || "Too many requests. Please wait a few minutes and try again."
      });
      return;
    }
    entry.count += 1;
...
var forgotPasswordRateLimitMap = /* @__PURE__ */ new Map();
var forgotPasswordRateLimit = createIpRateLimit({
...
var supportTicketRateLimitMap = /* @__PURE__ */ new Map();
var supportTicketRateLimit = createIpRateLimit({
  bucket: supportTicketRateLimitMap,
  windowMs: 60 * 60 * 1e3,
  // 1 hour
  limit: 5,
  message: "Too many support requests. Please wait an hour and try again."
});
var publicBookingRateLimitMap = /* @__PURE__ */ new Map();
var publicBookingRateLimit = createIpRateLimit({
  bucket: publicBookingRateLimitMap,
  windowMs: 60 * 60 * 1e3,
  // 1 hour
  limit: 10,
  message: "Too many booking submissions. Please wait an hour and try again."
});
...
async function handleMarketplaceBooking(params) {
  ...
  let preferredTimes = [];
  let parsedPreferred = preferredTimesJson;
  ...
  if (Array.isArray(parsedPreferred)) {
    preferredTimes = parsedPreferred.filter(
      (s) => typeof s === "string"
    );
  }
  const firstPreferredDate = preferredTimes[0] ? new Date(preferredTimes[0]) : void 0;
  const validPreferredDate = firstPreferredDate && !isNaN(firstPreferredDate.getTime()) ? firstPreferredDate : void 0;
  let submission;
  let clientId;
  let job;
  let appointmentId;
  await db.transaction(async (tx) => {
    const [sub] = await tx.insert(intakeSubmissions).values({
      ...
      status: isInstant ? "confirmed" : "submitted"
    }).returning();
    submission = sub;
    if (clientEmail) {
    ...
    if (isInstant) {
      const apptDate = validPreferredDate ?? /* @__PURE__ */ new Date();
      let appt;
      if (homeownerUserId) {
      ...
      if (!appt) {
        const inserted = await tx.insert(appointments).values({
          ...
          status: "confirmed"
        }).onConflictDoNothing().returning({ id: appointments.id });
        if (inserted[0]) {
          appt = inserted[0];
  ...
  const preferredTimeStr = validPreferredDate ? validPreferredDate.toLocaleTimeString([], {
    hour: "2-digit",
    minute: "2-digit"
  }) : void 0;
  if (providerRow?.userId) {
    const inAppTitle = isInstant ? "New Booking Confirmed" : "New Booking Request";
    const inAppMessage = isInstant ? `${clientName} booked ${link.customTitle ?? "a service"}. View details in Schedule.` : `${clientName} submitted a new booking request. Review it in Intake Submissions.`;
    dispatchNotification(
      providerRow.userId,
      inAppTitle,
      inAppMessage,
      ...
      (e) => console.error("provider in-app/push dispatch error:", e)
    );
  }
  if (isInstant) {
    if (clientEmail || providerRow?.email) {
      dispatch("booking.created", {
        ...
        providerName,
        providerUserId: providerRow?.userId ?? void 0,
        recipientUserId: homeownerUserId || void 0,
        serviceName: link.customTitle ?? "Home Service",
    ...
    if (providerRow?.email) {
      dispatch("booking.request_provider", {
        ...
        (e) => console.error("booking.request_provider dispatch error:", e)
      );
    }
    if (clientEmail) {
      dispatch("booking.request_received", {
        ...
        providerName,
        recipientUserId: homeownerUserId || void 0,
        serviceName: link.customTitle ?? void 0,
        ...
        (e) => console.error("booking.request_received dispatch error:", e)
      );
    }
  }
  return {
...
function detectServiceCategory(title) {
  ...
  if (t.includes("hvac") || t.includes("heat") || t.includes("air") || t.includes("furnace") || t.includes("ac ") || t.includes("cooling"))
    return "HVAC";
  if (t.includes("plumb") || t.includes("pipe") || t.includes("water") || t.includes("drain") || t.includes("toilet") || t.includes("faucet"))
    return "Plumbing";
  if (t.includes("electr") || t.includes("wiring") || t.includes("outlet") || t.includes("circuit"))
    return "Electrical";
  if (t.includes("roof") || t.includes("gutter") || t.includes("shingle"))
    return "Roof";
  if (t.includes("pest") || t.includes("termite") || t.includes("rodent") || t.includes("insect"))
    return "Pest Control";
  if (t.includes("lawn") || t.includes("garden") || t.includes("landscap") || t.includes("grass") || t.includes("mow"))
    return "Lawn";
  if (t.includes("paint") || t.includes("coat")) return "Painting";
  if (t.includes("clean")) return "Cleaning";
  if (t.includes("appliance") || t.includes("washer") || t.includes("dryer") || t.includes("dishwash") || t.includes("refriger"))
    return "Appliances";
  return "General";
}
...
async function calculateAndPersistHouseFaxScore(homeId) {
  ...
  const withPhotos = allEntries.filter(
    (e) => Array.isArray(e.photos) && e.photos.length > 0
  ).length;
...
async function registerRoutes(app2) {
  ...
  function recordLoginSuccess(keys) {
  ...
  app2.post("/api/auth/forgot-password", forgotPasswordRateLimit, async (req, res) => {
    try {
      ...
      if (user) {
        ...
        sendPasswordResetEmail2(user.email, fullName, resetUrl).catch(
          (err) => {
  ...
  app2.delete(
    ...
    async (req, res) => {
      try {
        ...
        await db.transaction(async (tx) => {
          const providerRow = await tx.select({
            id: providers.id,
  ...
  app2.post(
    ...
    async (req, res) => {
      try {
        ...
        const prefix = ALLOWED_MIME_PREFIXES_AVATAR.find(
          (p) => base64.startsWith(p)
        );
        if (!prefix) {
        ...
        if (supabaseClient) {
          const { error: uploadError } = await supabaseClient.storage.from("job-photos").upload(`avatars/${filename}`, buffer, {
          ...
          const { data: publicUrlData } = supabaseClient.storage.from("job-photos").getPublicUrl(`avatars/${filename}`);
        ...
        enrichPropertyData(fullAddress).then(async (enrichment) => {
          try {
            ...
            if (enrichment.zillow) {
              ...
              if (z2.bedrooms) zillowUpdates.bedrooms = z2.bedrooms;
              if (z2.bathrooms) zillowUpdates.bathrooms = z2.bathrooms;
              if (z2.livingArea) zillowUpdates.squareFeet = z2.livingArea;
              if (z2.yearBuilt) zillowUpdates.yearBuilt = z2.yearBuilt;
              if (z2.lotSize) zillowUpdates.lotSize = z2.lotSize;
              if (z2.propertyType) zillowUpdates.propertyType = z2.propertyType;
              if (z2.zestimate)
                zillowUpdates.estimatedValue = String(z2.zestimate);
              if (z2.zpid) zillowUpdates.zillowId = z2.zpid;
              if (z2.url) zillowUpdates.zillowUrl = z2.url;
              if (z2.taxAssessedValue)
                zillowUpdates.taxAssessedValue = String(z2.taxAssessedValue);
              if (z2.lastSoldDate)
                zillowUpdates.lastSoldDate = z2.lastSoldDate;
              if (z2.lastSoldPrice)
                zillowUpdates.lastSoldPrice = String(z2.lastSoldPrice);
              if (z2.stories) zillowUpdates.stories = z2.stories;
            }
    ...
    async (req, res) => {
      try {
        ...
        if (enrichment.zillow) {
          ...
          if (z2.lotSize) zillowUpdates.lotSize = z2.lotSize;
          if (z2.propertyType) zillowUpdates.propertyType = z2.propertyType;
          if (z2.zestimate) zillowUpdates.estimatedValue = String(z2.zestimate);
          if (z2.zpid) zillowUpdates.zillowId = z2.zpid;
          if (z2.url) zillowUpdates.zillowUrl = z2.url;
          if (z2.taxAssessedValue)
            zillowUpdates.taxAssessedValue = String(z2.taxAssessedValue);
          if (z2.lastSoldDate) zillowUpdates.lastSoldDate = z2.lastSoldDate;
          if (z2.lastSoldPrice)
            zillowUpdates.lastSoldPrice = String(z2.lastSoldPrice);
          if (z2.stories) zillowUpdates.stories = z2.stories;
        }
        ...
        res.json({
          ...
          enrichment,
          fieldsUpdated: Object.keys(zillowUpdates).length + Object.keys(otherUpdates).length - 1
        });
      } catch (error) {
  ...
  app2.patch(
    ...
    async (req, res) => {
      try {
        ...
        const sourceParam = req.body?.source || "homeowner";
        const allowedSources = /* @__PURE__ */ new Set([
          "homeowner",
          "health_score",
          "survival_kit"
        ]);
        const source = allowedSources.has(sourceParam) ? sourceParam : "homeowner";
      ...
      try {
        ...
        const assets = Array.from(systemMap.entries()).map(([system, data]) => {
          ...
          const nextDueDate = new Date(
            data.lastServiced.getTime() + intervalMonths * 30 * 24 * 60 * 60 * 1e3
          );
          return {
            system,
        ...
        const documentsFromInvoices = allInvoices.map((inv) => {
          ...
          return {
            ...
            type: "invoice",
            date: inv.paidAt ? new Date(inv.paidAt).toLocaleDateString("en-US", {
              month: "short",
              year: "numeric"
...
Give actionable, specific recommendations. Be brief (1 sentence each).`;
    ...
    async (req, res) => {
      ...
      try {
        ...
        for (const photo of photos) {
          const validPrefix = ALLOWED_MIME_PREFIXES.find(
            (p) => photo.startsWith(p)
          );
          if (!validPrefix) {
        ...
        for (const photo of photos) {
          const prefix = ALLOWED_MIME_PREFIXES.find(
            (p) => photo.startsWith(p)
          );
          ...
          if (supabaseClient) {
            const { data: uploadData, error: uploadError } = await supabaseClient.storage.from("job-photos").upload(`photos/${filename}`, buffer, {
              contentType: mimeType,
              upsert: false
            });
            if (uploadError) {
    ...
    async (req, res) => {
      try {
        ...
        if (linkedJob && linkedJob.status !== "completed") {
          ...
          if (completedJob) {
            autoLogHouseFaxEntry(completedJob).catch(
              (e) => console.error("housefax auto-log error:", e)
            );
          }
        } else {
          const { finalPrice } = req.body;
        ...
        sendReviewNudge(req.params.id).catch(
          (e) => console.error("review nudge dispatch error:", e)
        );
  ...
  app2.get("/api/provider-resources", async (_req, res) => {
    try {
      const { getProviderResources: getProviderResources2 } = await Promise.resolve().then(() => (init_providerResources(), providerResources_exports));
      const { resources, source, fetchedAt } = await getProviderResources2();
  ...
  app2.get("/api/providers", async (req, res) => {
    try {
      ...
      const withPartner = providersList.map((p) => ({
        ...p,
      ...
      const enriched = withPartner.map((p) => {
        const pLat = p.latitude !== null && p.latitude !== void 0 ? parseFloat(p.latitude) : NaN;
        const pLng = p.longitude !== null && p.longitude !== void 0 ? parseFloat(p.longitude) : NaN;
  ...
  app2.get(
    ...
    async (req, res) => {
      try {
        ...
        const providerServices2 = await storage.getProviderServices(
          req.params.id
        );
        ...
        const userLat = latRaw !== void 0 ? parseFloat(latRaw) : NaN;
        const userLng = lngRaw !== void 0 ? parseFloat(lngRaw) : NaN;
        const pLat = provider.latitude !== null && provider.latitude !== void 0 ? parseFloat(provider.latitude) : NaN;
        const pLng = provider.longitude !== null && provider.longitude !== void 0 ? parseFloat(provider.longitude) : NaN;
        const distance = Number.isFinite(userLat) && Number.isFinite(userLng) && Number.isFinite(pLat) && Number.isFinite(pLng) ? Math.round(haversineMiles(userLat, userLng, pLat, pLng) * 10) / 10 : null;
        res.json({
          provider: {
            ...provider,
            bookingPolicies,
  ...
  app2.post(
    ...
    async (req, res) => {
      try {
        ...
        await db.insert(savedProviders).values({ userId: authUserId, providerId: req.params.providerId }).onConflictDoNothing();
  ...
  app2.post(
    ...
    async (req, res) => {
      try {
        ...
        const details = req.body?.details ? String(req.body.details).trim().slice(0, 1e3) : null;
        if (!reason) {
    ...
    async (req, res) => {
      try {
        ...
        if (!linkedInvoice && appointment.userId) {
          ...
          const visible = candidateInvoices.filter(
            (inv) => VISIBLE_STATUSES.has(inv.status)
          );
          const apptTime = appointment.scheduledDate ? new Date(appointment.scheduledDate).getTime() : null;
          const sixtyDays = 60 * 24 * 60 * 60 * 1e3;
          const payable = visible.filter(
            (inv) => inv.status === "sent" || inv.status === "overdue"
          );
          const nearPayable = apptTime ? payable.find((inv) => {
            const dt = inv.dueDate ? new Date(inv.dueDate).getTime() : 0;
            return dt && Math.abs(dt - apptTime) <= sixtyDays;
          }) : null;
          linkedInvoice = nearPayable ?? payable[0] ?? visible[0] ?? null;
        }
        if (!linkedInvoice && appointment.userId) {
          ...
          if (clientIds.length > 0) {
            ...
            const visible = legacyCandidates.filter(
              (inv) => VISIBLE_STATUSES.has(inv.status)
            );
            const apptTime = appointment.scheduledDate ? new Date(appointment.scheduledDate).getTime() : null;
            const sixtyDays = 60 * 24 * 60 * 60 * 1e3;
        ...
        if (linkedInvoice && !linkedInvoice.hostedInvoiceUrl && linkedInvoice.status !== "draft" && linkedInvoice.status !== "cancelled") {
          try {
        ...
        res.json({
          appointment: {
            ...
            job: linkedJob ? {
              ...
              checklistCount: Array.isArray(linkedJob.checklist) ? linkedJob.checklist.length : 0
            } : null,
  ...
  app2.post(
    ...
    async (req, res) => {
      try {
        ...
        const { appointment, created: appointmentCreated } = await storage.createAppointment(parsed.data);
        ...
        if (depositAmountCents > 0) {
          ...
          try {
            const session = await createDepositCheckoutSession({
              ...
              description: `${parsed.data.serviceName} on ${typeof parsed.data.scheduledDate === "string" ? parsed.data.scheduledDate : parsed.data.scheduledDate.toISOString().slice(0, 10)} at ${parsed.data.scheduledTime}`
            });
            depositCheckoutUrl = session.checkoutUrl;
          } catch (depositErr) {
            const code = typeof depositErr === "object" && depositErr !== null && "code" in depositErr ? depositErr.code : void 0;
        ...
        if (clientId) {
          try {
            ...
            const apptAddOns = Array.isArray(req.body.addOns) ? req.body.addOns.filter(
              ...
              price: typeof a.price === "number" ? a.price : parseFloat(String(a.price ?? "0")) || 0
            })).filter((a) => a.name.length > 0) : [];
            const baseApptServiceName = (parsed.data.serviceName ?? "").split(" + ")[0].trim();
            ...
            await db.insert(jobs).values({
              ...
              description: composedApptDescription || parsed.data.description || null,
        ...
        if (bookedUser && bookedProvider && !depositPending) {
          ...
          const derivedAddOnNames = (() => {
            if (Array.isArray(explicitAddOns) && explicitAddOns.length > 0) {
              return explicitAddOns.map(
                (a) => typeof a === "string" ? a : typeof a === "object" && a !== null && "name" in a ? String(a.name) : null
              ).filter((n) => Boolean(n));
            }
            ...
            if (appointment.notes) {
              try {
                ...
                if (notesData.addOns && Array.isArray(notesData.addOns)) {
                  const names = notesData.addOns.map(
                    (a) => typeof a === "string" ? a : typeof a === "object" && a !== null && "name" in a ? String(a.name) : null
                  ).filter(
          ...
          const bodyServiceId = typeof req.body.customServiceId === "string" ? req.body.customServiceId : void 0;
          ...
          let intakeAnswersSummary;
          const formatAnswersObj = (obj) => {
          ...
          dispatch("booking.created", {
            ...
            serviceDescription: matchedSvc?.description ?? void 0,
      ...
      try {
        ...
        if (updatedApptUser && updatedApptProvider) {
          dispatch("booking.updated", {
            ...
            (e) => console.error("booking.updated dispatch error:", e)
          );
        }
      ...
      try {
        ...
        if (cancelledUser && cancelledProvider) {
          dispatch("booking.cancelled", {
            ...
            (e) => console.error("booking.cancelled dispatch error:", e)
          );
        }
        res.json({
          appointment,
      ...
      try {
        ...
        if (rescheduledUser && rescheduledProvider) {
          dispatch("booking.rescheduled", {
            ...
            (e) => console.error("booking.rescheduled dispatch error:", e)
          );
        }
        res.json({ appointment });
      } catch (error) {
  ...
  app2.post(
    ...
    async (req, res) => {
      try {
        ...
        const messages = [{ role: "system", content: INTAKE_SYSTEM_PROMPT }];
...
- confidence: 0-100 confidence in these estimates`;
        const response = await openai.chat.completions.create({
          ...
          messages: [
            {
              role: "system",
              content: "You are HomeBase's pricing AI. Generate realistic estimates with service package options to help pros close leads. Respond with valid JSON only."
            },
  ...
  app2.post(
    ...
    async (req, res) => {
      try {
        ...
        const answersSummary = Object.entries(answers).map(
...
Respond with JSON only:
{
  ...
  "priceRange": { "min": number, "max": number }
        ...
        const response = await openai.chat.completions.create({
          ...
          messages: [
            {
              role: "system",
              content: "You are a home services expert helping homeowners understand their issues. Be clear, helpful, and reassuring."
            },
  ...
  app2.post(
    ...
    async (req, res) => {
      try {
        ...
        const { title, description, category, frequency, nextDueAt } = req.body;
        const userId = req.authenticatedUserId;
  ...
  app2.put(
    ...
    async (req, res) => {
      try {
        ...
        if (recurringFrequency !== void 0)
          allowedUpdate.recurringFrequency = recurringFrequency;
        if (recurringPrice !== void 0)
      ...
      try {
        ...
        if (update.isPublic === true) {
          ...
          if (!ready) {
            return res.status(422).json({
              ...
              message: "Set up payments with Stripe before publishing your profile so clients can book and pay you."
            });
          }
        }
        if (update.isActive === true) {
          ...
          if (!ready) {
            return res.status(422).json({
              error: "stripe_not_ready",
              message: "Finish Stripe setup to start accepting bookings."
            });
          }
        }
        if (body.bookingPolicies !== void 0 || body.instantBooking !== void 0 || body.advanceBookingDays !== void 0) {
          ...
          if (body.instantBooking !== void 0)
            merged.instantBooking = body.instantBooking;
          if (body.advanceBookingDays !== void 0)
  ...
  app2.post(
    ...
    async (req, res) => {
      try {
        ...
        const prefix = ALLOWED_MIME_PREFIXES_LOGO.find(
          (p) => base64.startsWith(p)
        );
        if (!prefix) {
          return res.status(400).json({ error: "Invalid image format. Use JPEG, PNG, or WebP." });
        ...
        if (supabaseClient) {
          const { error: uploadError } = await supabaseClient.storage.from("job-photos").upload(`logos/${filename}`, buffer, {
            contentType: mimeType,
            upsert: true
          });
  ...
  const REPLY_MAX_LENGTH = 1e3;
  async function loadReviewForReply(reviewId, authUserId) {
    const [row] = await db.select({
      review: reviews,
      provider: providers
  ...
  app2.get(
    ...
    async (req, res) => {
      try {
        ...
        const completedJobsTotal = jobs2.reduce((sum, j) => {
      ...
      if (housefaxData && typeof housefaxData === "object" && req.authenticatedUserId) {
        try {
          ...
          const newHome = await storage.createHome({
            ...
            bedrooms: housefaxData.bedrooms ?? void 0,
            bathrooms: housefaxData.bathrooms ?? void 0,
            squareFeet: housefaxData.squareFeet ?? void 0,
            yearBuilt: housefaxData.yearBuilt ?? void 0,
            lotSize: housefaxData.lotSize ?? void 0,
            estimatedValue: housefaxData.estimatedValue ? String(housefaxData.estimatedValue) : void 0,
            zillowId: housefaxData.zillowId ?? void 0,
            zillowUrl: housefaxData.zillowUrl ?? void 0,
            taxAssessedValue: housefaxData.taxAssessedValue ? String(housefaxData.taxAssessedValue) : void 0,
            lastSoldDate: housefaxData.lastSoldDate ?? void 0,
            lastSoldPrice: housefaxData.lastSoldPrice ? String(housefaxData.lastSoldPrice) : void 0,
            latitude: housefaxData.latitude ? String(housefaxData.latitude) : void 0,
            longitude: housefaxData.longitude ? String(housefaxData.longitude) : void 0,
            placeId: housefaxData.placeId ?? void 0,
            formattedAddress: housefaxData.formattedAddress ?? void 0,
            neighborhoodName: housefaxData.neighborhoodName ?? void 0,
            countyName: housefaxData.countyName ?? void 0,
            housefaxEnrichedAt: /* @__PURE__ */ new Date()
          });
          homeId = newHome.id;
        } catch (homeErr) {
    ...
    async (req, res) => {
      try {
        ...
        if (!invoice) {
          ...
          if (linkedJob?.appointmentId) {
            ...
            if (appt?.userId) {
              ...
              const apptTime = appt.scheduledDate ? new Date(appt.scheduledDate).getTime() : null;
              const sixtyDays = 60 * 24 * 60 * 60 * 1e3;
              const payable = visible.filter(
                (inv) => inv.status === "sent" || inv.status === "overdue"
              );
              ...
              invoice = nearPayable ?? payable[0] ?? visible[0] ?? void 0;
            }
            if (!invoice && appt?.userId) {
              ...
              if (clientIds.length > 0) {
                ...
                invoice = payable[0] ?? visible[0] ?? void 0;
              }
            }
          }
        }
        ...
        if (!isHomeowner && !isProvider)
          return res.status(403).json({ error: "Access denied" });
        if (!invoice.hostedInvoiceUrl && invoice.status !== "draft" && invoice.status !== "cancelled") {
          try {
  ...
  async function dispatchJobStatusEmail(job, newStatus, extra) {
    ...
    await dispatch("job.status_changed", {
      clientEmail,
      clientName,
      clientPhone: client.phone ?? void 0,
      providerName: provider.businessName,
  ...
  app2.post("/api/jobs", requireAuth, async (req, res) => {
    try {
      ...
      let verifiedCustomServiceId = null;
      ...
      const rawJobAddOns = Array.isArray(req.body.selectedAddOns) ? req.body.selectedAddOns.filter(
        (a) => typeof a === "object" && a !== null
      ).map((a) => ({
        name: String(a.name ?? "").slice(0, 200),
        price: typeof a.price === "number" ? a.price : parseFloat(String(a.price ?? "0")) || 0
      ...
      const finalJobDescription = composedJobDescription || parsed.data.description || null;
      const effectivePrice = parsed.data.estimatedPrice || (svcSnapshot?.pricingType === "fixed" || svcSnapshot?.pricingType === "service_call" ? svcSnapshot?.basePrice ?? void 0 : void 0) || (svcSnapshot?.pricingType === "variable" ? svcSnapshot?.priceFrom ?? void 0 : void 0);
      let resolvedHomeownerUserId = null;
      let resolvedHomeId = null;
      if (parsed.data.clientId) {
        const [jobClientRow] = await db.select({
          id: clients.id,
          homeId: clients.homeId,
          homeownerUserId: clients.homeownerUserId
        }).from(clients).where(
        ...
        if (!jobClientRow) {
          return res.status(403).json({ error: "Forbidden: client does not belong to this provider" });
        }
      ...
      const { job: newJob, appointment } = await db.transaction(async (tx) => {
        ...
        const [apptRow] = await tx.insert(appointments).values({
          userId: resolvedHomeownerUserId ?? void 0,
          homeId: resolvedHomeId ?? void 0,
          providerId: job.providerId,
          serviceName: job.title,
          ...
          try {
            ...
            if (jobClient?.email && jobProvider) {
              ...
              if (Array.isArray(req.body.selectedAddOns)) {
                ...
                try {
                  const parsed2 = JSON.parse(req.body.selectedAddOns);
                  if (Array.isArray(parsed2)) rawAddOns = parsed2;
                } catch {
                }
              }
              const selectedAddOns = rawAddOns ? rawAddOns.filter(
                (a) => typeof a === "object" && a !== null
              ).map((a) => ({
                name: String(a.name ?? "").slice(0, 200),
                price: Math.max(0, Number(a.price ?? 0))
              })).filter((a) => a.name.length > 0) : void 0;
              ...
              const rawPricingType = typeof req.body.pricingType === "string" ? req.body.pricingType : void 0;
              const pricingTypeFromBody = rawPricingType && ALLOWED_PRICING_TYPES.includes(rawPricingType) ? rawPricingType : void 0;
              const rawServiceDesc = typeof req.body.serviceDescription === "string" ? req.body.serviceDescription.trim() : void 0;
              const serviceDescFromBody = rawServiceDesc ? rawServiceDesc.slice(0, 1e3) : void 0;
              await sendProviderScheduledJobEmail({
                clientEmail: jobClient.email,
                clientName,
                providerName: jobProvider.businessName,
                providerPhone: jobProvider.phone || void 0,
                providerEmail: jobProvider.email || void 0,
                serviceName: newJob.title,
                scheduledDate: scheduledDateStr,
                scheduledTime: scheduledTimeStr,
                address: newJob.address || jobClient.address || void 0,
                estimatedPrice: newJob.estimatedPrice || void 0,
                description: newJob.description || void 0,
                serviceDescription: fullSvcData?.description || serviceDescFromBody || void 0,
                pricingType: fullSvcData?.pricingType || pricingTypeFromBody || void 0,
  ...
  app2.put(
    ...
    async (req, res) => {
      try {
        ...
        if (status === "completed" && existing.status !== "completed" && job.appointmentId) {
          ...
          (async () => {
            try {
              ...
              if (appt) {
                ...
                if (appt.status !== "cancelled" && !REVIEWABLE.has(appt.status || "")) {
    ...
    async (req, res) => {
      try {
        ...
        if (prior?.status !== "completed") {
          dispatchJobStatusEmail(job, "completed").catch(
            (e) => console.error("job.status_changed dispatch error:", e)
          );
        }
        (async () => {
        ...
        autoLogHouseFaxEntry(job).catch(
          (e) => console.error("housefax auto-log error:", e)
        );
        if (job.appointmentId) {
          const apptId = job.appointmentId;
  ...
  app2.post(
    ...
    async (req, res) => {
      try {
        ...
        if (req.body.providerId && !await assertProviderOwnership(req, req.body.providerId, res))
          return;
        if (req.body.providerId && !await checkSubscriptionGate(req.body.providerId, res))
          return;
        ...
        const lineItemsJson = lineItemsInput.length > 0 ? JSON.stringify(lineItemsInput) : req.body.amount ? JSON.stringify([
          {
        ...
        const draftPlan = req.body.providerId ? await getProviderPlan(req.body.providerId) : { platformFeePercent: "3.00", platformFeeFixedCents: 0 };
        ...
        if (invoice.clientId) {
          ...
          if (draftClient?.email && draftProvider) {
            dispatch("invoice.created", {
              ...
              (e) => console.error("invoice.created dispatch error:", e)
            );
          }
        }
      ...
      try {
        ...
        if (stripeErrorCode === "stripe_not_ready") {
          return res.status(409).json({
            ...
            error: "Provider Stripe Connect account is not ready to accept charges. Finish onboarding before sending invoices.",
            invoiceId: invoice.id
          });
        }
        ...
        if (invoice.clientId) {
          ...
          if (client?.email && provider) {
            ...
            notifyHomeownerInvoiceSent({
              ...
              (e) => console.error("[invoice.sent] homeowner notify failed:", e)
            );
    ...
    async (req, res) => {
      try {
        ...
        if (!authProviderRecord || invoice.providerId !== authProviderRecord.id) {
          return res.status(403).json({
            error: "Access denied: you can only send invoices for your own provider account"
          });
        }
        let hostedUrl;
        let stripeError;
  ...
  app2.post(
    ...
    async (req, res) => {
      try {
        ...
        res.json({ invoice });
        if (invoice) {
          maybeStartGracePeriod(invoice.providerId).catch(
            (e) => console.error("[subscription] grace start failed:", e)
          );
        }
    ...
    async (req, res) => {
      try {
        ...
        notifyHomeownerInvoiceReminder({
          ...
          (e) => console.error("[invoice.reminder] homeowner notify failed:", e)
        );
        res.json({ sent: true });
      } catch (error) {
    ...
    async (req, res) => {
      try {
        ...
        if (!invoice)
          return res.status(404).json({ error: "Invoice not found" });
        const authProvider = await storage.getProviderByUserId(authUserId);
        const isProvider = !!authProvider && invoice.providerId === authProvider.id;
        let isHomeowner = invoice.homeownerUserId === authUserId;
  ...
  app2.post(
    ...
    async (_req, res) => {
      res.status(410).json({
        ...
        message: "This endpoint was removed. Use /api/invoices/:invoiceId/checkout for provider invoice payments."
      });
    }
  );
  app2.post(
  ...
  function rejectIosSubscriptionFlow(req, res) {
    const platform = String(
      req.headers["x-client-platform"] || ""
    ).toLowerCase();
    if (platform === "ios") {
      res.status(403).json({
        error: "Subscriptions on iOS must be purchased through the App Store. Open the Subscription screen in the app to subscribe.",
        code: "ios_use_iap"
      });
      return true;
    }
  ...
  app2.post("/api/revenuecat/webhook", async (req, res) => {
    const { handleRevenueCatWebhook: handleRevenueCatWebhook2 } = await Promise.resolve().then(() => (init_revenuecatService(), revenuecatService_exports));
    await handleRevenueCatWebhook2(req, res);
  });
...
<body>
  ...
  const invoiceReturnHtml = (invoiceId, jobId, status) => {
    ...
    const message = isPaid ? "Thanks \u2014 your payment was processed successfully. Returning you to the HomeBase app." : "No charge was made. You can return to the HomeBase app to try again or pick a different payment method.";
    const icon = isPaid ? "\u2713" : "\u2715";
...
<body>
  ...
  app2.get(
    ...
    async (req, res) => {
      ...
      const jobId = await lookupInvoiceJobId2(invoiceId);
        ...
        if (!url) {
          try {
            ...
            if (err?.code === "stripe_not_ready") {
              return res.status(409).json({
                code: "stripe_not_ready",
                error: "Provider Stripe Connect account is not ready to accept charges.",
                invoiceId
              });
            }
            throw err;
  ...
  app2.post(
    ...
    async (req, res) => {
      try {
        ...
        if (platformFeePercent !== void 0 || platformFeeFixedCents !== void 0) {
          return res.status(403).json({
  ...
  app2.post(
    ...
    async (req, res) => {
      try {
        ...
        res.status(500).json({ error: error.message || "Failed to update provider fees" });
      }
    }
  );
  app2.get(
  ...
  app2.post(
    ...
    async (req, res) => {
      try {
        ...
        if (error?.code === "stripe_not_ready" || error?.message?.includes("not enabled") || error?.message?.includes("not set up") || error?.message?.includes("Stripe Connect onboarding") || error?.message?.includes("charges are not enabled")) {
  ...
  app2.get(
    ...
    async (req, res) => {
      try {
        ...
        const stripe2 = getStripe();
        const pms = await stripe2.paymentMethods.list({
          customer: user.stripeCustomerId,
          type: "card"
        });
        const customer = await stripe2.customers.retrieve(
  ...
  app2.post(
    ...
    async (req, res) => {
      try {
        ...
        console.error("Create Stripe invoice error:", error);
        if (error?.code === "stripe_not_ready" || error?.message?.includes("not enabled") || error?.message?.includes("not set up") || error?.message?.includes("Stripe Connect onboarding")) {
          return res.status(402).json({
            error: "stripe_not_ready",
            message: error.message
          });
    ...
    async (req, res) => {
      try {
        ...
        res.status(201).json({
          ...
          message: result.instantBooking ? "Your booking has been confirmed!" : "Your request has been submitted!"
        });
      } catch (error) {
    ...
    async (req, res) => {
      try {
        ...
        try {
          ...
          const apptTimeStr = job.scheduledTime || apptDate.toLocaleTimeString([], {
            hour: "2-digit",
            minute: "2-digit"
          });
          if (submission.clientEmail || providerRow?.email) {
            dispatch("booking.created", {
              clientEmail: submission.clientEmail || void 0,
              clientName: submission.clientName,
              providerEmail: providerRow?.email ?? void 0,
              providerName,
              providerUserId: providerRow?.userId ?? void 0,
              recipientUserId: submission.homeownerUserId || void 0,
              serviceName: job.title || "Home Service",
              appointmentDate: apptDateStr,
              appointmentTime: apptTimeStr,
              address: submission.address || void 0,
          ...
          if (submission.homeownerUserId) {
            dispatchNotification(
              ...
              (e) => console.error("Accept homeowner push error:", e)
            );
          }
        } catch (notifyErr) {
          console.error(
    ...
    async (req, res) => {
      try {
        ...
        if (!checkMessageRateLimit(providerId, clientId)) {
          return res.status(429).json({
            error: "Rate limit exceeded: max 10 messages per client per 24 hours"
          });
        }
        ...
        if (jobId) {
          ...
          if (jobRecord) {
            processedBody = processedBody.replace(/\{\{service\}\}/g, jobRecord.title || "").replace(
              /\{\{booking_date\}\}/g,
              jobRecord.scheduledDate ? new Date(jobRecord.scheduledDate).toLocaleDateString(
                "en-US",
                { month: "long", day: "numeric", year: "numeric" }
              ) : ""
            );
            processedSubject = processedSubject.replace(/\{\{service\}\}/g, jobRecord.title || "").replace(
              /\{\{booking_date\}\}/g,
              jobRecord.scheduledDate ? new Date(jobRecord.scheduledDate).toLocaleDateString() : ""
            );
          }
  ...
  app2.post(
    ...
    async (req, res) => {
      try {
        ...
        const validatedChannels = channels.filter(
  ...
  app2.post(
    ...
    async (req, res) => {
      try {
        ...
        const validatedChannels = channels.filter(
          (ch) => VALID_CHANNELS_BROADCAST.includes(ch)
        );
        if (validatedChannels.length === 0) {
          return res.status(400).json({
            error: "channels must include at least one of: push, email"
  ...
  app2.post("/api/support/ticket", supportTicketRateLimit, async (req, res) => {
    try {
      const { name, email, category, subject, message } = req.body;
      if (!name || !email || !category || !subject || !message) {
        return res.status(400).json({ error: "All fields are required" });
      }
      if (String(name).length > 200 || String(email).length > 254 || String(category).length > 100 || String(subject).length > 300 || String(message).length > 5e3) {
...
async function runBootMigrations() {
  ...
  try {
    ...
    await runSql(
      "stripe_connect_accounts.livemode",
...
var APP_STORE_URL = "https://apps.apple.com/app/homebase-pro-app/id6760936703";
...
function shellHtml(title, body, extraScript = "") {
  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
...
function summaryRows(rows) {
  ...
  return `<div class="summary">${visible.map(
    (r) => `<div class="summary-row"><span class="summary-label">${escapeHtml2(r.label)}</span><span class="summary-value">${escapeHtml2(r.value)}</span></div>`
  ).join("")}</div>`;
}
...
async function paymentSuccessHandler(req, res) {
  const jobIdRaw = req.query.jobId?.trim() || null;
  let jobId = jobIdRaw;
  const invoiceId = req.query.invoiceId?.trim() || null;
  const subscription = req.query.subscription?.trim() || null;
  ...
  const deepLink = isSubscription ? buildAppDeepLink({ base: "homebase://subscription", subscription: "success" }) : buildAppDeepLink({ base: jobId ? `homebase://job/${encodeURIComponent(jobId)}` : "homebase://payment-result", jobId, status: "paid", extra: { invoiceId } });
  const titleText = isSubscription ? "Subscription Active" : "Payment Received";
  const lede = isSubscription ? "You're all set \u2014 your HomeBase Pro subscription is now active. Open the app to start using your Pro features." : "Thanks! Your payment was processed successfully.";
  const summaryHtml = summary ? summaryRows([
  ...
  const body = `
    <div class="card">
      ...
      <h1>${escapeHtml2(titleText)}</h1>
      <p class="lede">${escapeHtml2(lede)}</p>
      ${summaryHtml}
      <div class="actions">
        <a id="open-app-btn" class="btn btn-primary" href="${escapeHtml2(deepLink)}" data-mobile-only>Open in App</a>
        <a class="btn btn-primary" href="${escapeHtml2(deepLink)}" data-desktop-only>${escapeHtml2(isSubscription ? "Continue" : "View Job")}</a>
        <div class="stores" data-desktop-only>
          <a class="btn btn-secondary" href="${escapeHtml2(APP_STORE_URL)}">App Store</a>
          <a class="btn btn-secondary" href="${escapeHtml2(PLAY_STORE_URL)}">Google Play</a>
        </div>
      </div>
...
async function paymentCancelledHandler(req, res) {
  const jobId = req.query.jobId?.trim() || null;
  const invoiceId = req.query.invoiceId?.trim() || null;
  const subscription = req.query.subscription?.trim() || null;
  const returnUrlRaw = req.query.return?.trim() || null;
  let returnUrl = null;
  if (returnUrlRaw) {
    try {
  ...
  const deepLink = isSubscription ? buildAppDeepLink({ base: "homebase://subscription", subscription: "cancelled" }) : buildAppDeepLink({ base: jobId ? `homebase://job/${encodeURIComponent(jobId)}` : "homebase://payment-result", jobId, status: "cancelled", extra: { invoiceId } });
  ...
  const body = `
    <div class="card">
      ...
      <div class="actions">
        <a class="btn btn-primary" href="${escapeHtml2(tryAgainHref)}">Try Again</a>
        <a id="open-app-btn" class="btn btn-secondary" href="${escapeHtml2(deepLink)}" data-mobile-only>Back to App</a>
        <a class="btn btn-secondary" href="${escapeHtml2(HOMEPAGE_URL)}" data-desktop-only>Back to Homepage</a>
      </div>
    </div>
  `;
  res.setHeader("Content-Type", "text/html; charset=utf-8");
...
async function bookingSuccessHandler(req, res) {
  const appointmentId = req.query.appointmentId?.trim() || null;
  const queryService = req.query.service?.trim() || null;
  const queryProvider = req.query.provider?.trim() || null;
  const queryDate = req.query.date?.trim() || null;
  let summary = null;
  ...
  const body = `
    <div class="card">
      ...
      ${summaryHtml}
      <div class="actions">
        <a id="open-app-btn" class="btn btn-primary" href="${escapeHtml2(deepLink)}" data-mobile-only>View Appointment</a>
        <a class="btn btn-primary" href="${escapeHtml2(deepLink)}" data-desktop-only>Open in App</a>
        <div class="stores" data-desktop-only>
          <a class="btn btn-secondary" href="${escapeHtml2(APP_STORE_URL)}">App Store</a>
          <a class="btn btn-secondary" href="${escapeHtml2(PLAY_STORE_URL)}">Google Play</a>
        </div>
...
async function openAppHandler(req, res) {
  const jobId = req.query.jobId?.trim() || null;
  const appointmentId = req.query.appointmentId?.trim() || null;
  const status = req.query.status?.trim() || null;
  const path3 = req.query.path?.trim() || null;
  let base = "homebase://";
  if (path3) {
    const cleaned = path3.replace(/[^a-zA-Z0-9/_\-.]/g, "");
  ...
  const deepLink = buildAppDeepLink({ base, jobId: path3 ? jobId : null, appointmentId: path3 ? appointmentId : null, status });
  const body = `
    <div class="card">
      ${BRAND_HTML}
      <div data-mobile-only>
        ...
        <div class="actions">
          <a id="open-app-btn" class="btn btn-primary" href="${escapeHtml2(deepLink)}">Open App</a>
          <div class="stores">
            <a class="btn btn-secondary" href="${escapeHtml2(APP_STORE_URL)}">App Store</a>
            <a class="btn btn-secondary" href="${escapeHtml2(PLAY_STORE_URL)}">Google Play</a>
          </div>
      ...
      <div data-desktop-only>
        ...
        <div class="actions">
          <a class="btn btn-primary" href="${escapeHtml2(HOMEPAGE_URL)}">Visit Homepage</a>
          <div class="stores">
            <a class="btn btn-secondary" href="${escapeHtml2(APP_STORE_URL)}">App Store</a>
            <a class="btn btn-secondary" href="${escapeHtml2(PLAY_STORE_URL)}">Google Play</a>
          </div>
  ...
  res.setHeader("Content-Type", "text/html; charset=utf-8");
  res.setHeader("Cache-Control", "no-store");
  res.send(shellHtml("Open in HomeBase", body, deepLinkScript({ deepLink, fallback: "stores", autoOpen: true, delayMs: 2e3 })));
}
function registerRedirectPages(app2) {
  app2.get("/payment-success", (req, res) => {
    void paymentSuccessHandler(req, res);
  });
  app2.get("/payment-cancelled", (req, res) => {
    void paymentCancelledHandler(req, res);
  });
  app2.get("/booking-success", (req, res) => {
    void bookingSuccessHandler(req, res);
  });
  app2.get("/open-app", (req, res) => {
    void openAppHandler(req, res);
  });
}

// server/webhookSecrets.ts
var _legacySecretWarned = {};
function resolveWebhookSecret(endpoint) {
  const newName = endpoint === "platform" ? "STRIPE_WEBHOOK_SECRET_PLATFORM" : "STRIPE_WEBHOOK_SECRET_CONNECT";
  const oldName = endpoint === "platform" ? "STRIPE_WEBHOOK_SECRET" : "STRIPE_CONNECT_WEBHOOK_SECRET";
  ...
  if (fromOld) {
    if (!_legacySecretWarned[oldName]) {
      console.warn(
        `[webhook] DEPRECATED env var ${oldName} in use \u2014 please rename to ${newName}. The old name will continue to work for now but should be migrated.`
      );
...
async function handlePlatformWebhook(req, res) {
...
function setupStripeWebhook(app2) {
  ...
  app2.post("/api/stripe/webhook/platform", rawJson, handlePlatformWebhook);
  app2.post("/api/stripe/webhook", rawJson, handlePlatformWebhook);
}
function setupStripeConnectWebhook(app2) {
  const rawJson = express.raw({ type: "application/json" });
  app2.post("/api/stripe/webhook/connect", rawJson, handleConnectWebhook);
  app2.post("/api/webhooks/stripe-connect", rawJson, handleConnectWebhook);
}
...
async function initStripe() {
  const databaseUrl2 = process.env.SUPABASE_DATABASE_URL;
  if (!databaseUrl2) {
...
async function runSubscriptionGraceReminder() {
  try {
    const { sendGraceReminderNotification: sendGraceReminderNotification2 } = await Promise.resolve().then(() => (init_subscriptionService(), subscriptionService_exports));
    const result = await pool.query(`
    ...
    for (const row of result.rows) {
      ...
      const daysRemaining = Math.max(1, Math.ceil((ends.getTime() - Date.now()) / 864e5));
      try {
        await sendGraceReminderNotification2(row.provider_id, daysRemaining);
      } catch (e) {
...
async function runSubscriptionExpiredNotice() {
  try {
    const { sendGraceExpiredNotification: sendGraceExpiredNotification2 } = await Promise.resolve().then(() => (init_subscriptionService(), subscriptionService_exports));
    const result = await pool.query(`
    ...
    for (const row of result.rows) {
      try {
        await sendGraceExpiredNotification2(row.provider_id);
      } catch (e) {
...
function validateProductionEnv() {
  ...
  const isSet = (key) => Array.isArray(key) ? key.some((k) => !!process.env[k]) : !!process.env[key];
  const labelOf = (key) => Array.isArray(key) ? key.join(" or ") : key;
  if (IS_PROD2) {
    let fatal = false;
