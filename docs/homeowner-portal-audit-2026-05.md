...
The inventory below uses the literal `Pass / Partial / Fail` labels in the Status column and `N/A` in the Severity column where appropriate. Counts: **19 Pass, 7 Partial, 1 Fail** — Pass = rows 1–4, 7–11, 13–15, 18–20, 22–24; Partial = rows 5 (`AIChat`, guest reaches the screen but every send 401s), 6 (`SmartIntake`, same), 12, 16, 17, 25, 26 (`Budgeter`, guest-only stub); Fail = row 27 (`SavingsSpend`, typed but unregistered). Note: row 24 (`ServiceHistoryScreen`) is Pass on the client but flagged with a *server-side* High severity for the §4.18 IDOR.
...
| 5 | `AIChatScreen` | **Guest+Auth** (RootStack via `FindScreen` AI card `:368–372` + AppointmentDetail/Onboarding) | Partial | Low | Reachable to guests because the `FindScreen` "Ask HomeBase AI" card is *not* auth-gated (only the search panel is). The screen calls `POST /api/chat/simple` (`routes.ts:4862`), which is `requireAuth + aiRateLimit` (`routes.ts:294`) — so a guest lands here but every send returns 401 until they sign in. The screen does not gate the input itself, which is the Partial: either gate the input client-side for guests (best UX) or accept the 401 toast as the gate. |
| 6 | `SmartIntakeScreen` | **Guest+Auth** (RootStack — entered from `BudgeterScreen.tsx:85`, `AIChatScreen.tsx:120`, `HouseFaxScreen.tsx:370, 583`, `HealthScoreScreen.tsx:926`, all of which are guest-reachable) | Partial | Low | 4-step AI wizard (`/api/intake/analyze`, `/api/intake/refine`, `/api/intake/match-providers` at `routes.ts:5947 / 6012 / 6099`); all three protected by `requireAuth + aiRateLimit`. A guest can open the wizard and the `/intake/analyze` POST in step 1 immediately 401s — the screen does not pre-check auth. The booking step enforces auth client-side via `AccountGateModal`, but the earlier API calls already failed by then. Same fix as `AIChatScreen`. |
