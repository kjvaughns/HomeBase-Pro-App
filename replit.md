...
## Recent Changes
- 2026-05-03: Task #303 — Weather-cancel as its own state. New `weather_held` value on `job_status` enum + `jobs.weather_held_at` / `jobs.original_scheduled_at` columns (boot migration in `server/dbMigrations.ts`, file `migrations/0009_task303_weather_held.sql`). Two new endpoints in `server/routes.ts` — `POST /api/jobs/:id/weather-hold` (accepts optional `newDate`/`newTime`, snapshots the original slot once, idempotent on repeated holds, mirrors date/time onto the paired appointment inside a single DB transaction, dispatches `weather_held` customer notification through the existing dispatcher) and `POST /api/jobs/:id/restore` (idempotent, restores both date and `scheduledTime` from the snapshot, mirrors to the appointment in the same tx). New "Heads up — weather is moving us" copy in `server/notificationService.ts` + `server/emailService.ts`. Recurring "this+following" field/date shifts in `server/recurringJobsService.ts` and the custom-service template apply in `server/routes.ts` now exclude `weather_held`. Provider Job Detail screen ships a "Hold for weather" action (Hold only / Hold & move +7d) with a customer-message preview, plus a one-tap "Restore job" action when held. ScheduleScreen renders weather-held jobs with a `cloud-rain` glyph + neutral grey pill (accent #38AE5F preserved for active forward motion). Held jobs are excluded from cancellation/completion stats and from upcoming-jobs counts.
