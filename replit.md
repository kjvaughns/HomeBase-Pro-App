...
- **AI Service Blueprint Wizard**: Providers create services through a 6-step provider-first form builder (`ServiceBlueprintWizardScreen`). Philosophy: "Build your service your way, with smart suggestions from HomeBase." AI assists optionally — it never auto-runs. Steps: (0) Name your service — provider types service name, picks category from pills, adds optional description; (1) Your pricing — pick pricing type (Flat Rate / Starts At / Custom Quote) + price input + unit, with an optional "Get AI pricing suggestion" secondary link; (2) What customers need to tell you — manual question add/delete form with optional "Suggest questions for [service]" AI link surfacing tap-to-add chips; (3) Optional upgrades — manual add-on add/delete form with optional "Suggest upgrades" AI link; (4) How customers book — booking mode picker (Instant Book / Provider Confirms / Quote Only), locked to Quote Only when pricing type is Custom Quote; (5) Review your service — editable summary card with pencil-icon jumps back to any step, Preview modal, Publish button. AI blueprint (`POST /api/ai/service-blueprint`) is fetched lazily and cached on first "Suggest" tap. Manual entry escape hatch navigates to `EditService`. The `NewService` navigation route points to the wizard; `EditService` routes to the original `NewServiceScreen`. Four `provider_custom_services` columns: `intake_questions_json`, `add_ons_json`, `booking_mode` (instant/starts_at/quote_only), `ai_pricing_insight` (boot migration applied). `SimpleBookingScreen` surfaces intake questions as interactive fields and shows "Request a Quote" CTA when `bookingMode === "quote_only"`. `ServicesScreen` shows an eye-icon preview button per service. Public booking page (`/book/:slug`) renders intake questions and adapts CTA/price display for `quote_only` and `starts_at` booking modes.
